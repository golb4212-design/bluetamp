...
URL="https://raw.githubusercontent.com/$REPO/$BRANCH/index.html?v=6"
if [[ $EUID -ne 0 ]]; then echo "این اسکریپت را با sudo اجرا کنید."; exit 1; fi
mkdir -p "$TARGET_DIR"
if [[ -f "$TARGET_FILE" ]]; then cp -a "$TARGET_FILE" "$TARGET_FILE.backup.$(date +%Y%m%d-%H%M%S)"; fi
curl -4 -fL --connect-timeout 20 --retry 3 -H "Cache-Control: no-cache" "$URL" -o "$TARGET_FILE.tmp"
grep -q "BlueTamp PasarGuard Subscription Template v6.0.0" "$TARGET_FILE.tmp" || { echo "فایل دانلودشده نسخه ۵ نیست. ابتدا فایل جدید را در GitHub جایگزین کنید."; rm -f "$TARGET_FILE.tmp"; exit 1; }
